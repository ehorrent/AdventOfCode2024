...
impl Grid {
    ...
    fn get_cheats_count(&self, cheat_duration: usize, time_saved_min: usize) -> usize {
        ...
        }) = self.compute_best_path()
        {
            ...
            time_per_pos.insert(self.exit_pos, 0);
            ...
            for (path_time, cheat_start_pos) in ordered_path.iter().enumerate() {
                ...
                for cheat_end_pos in free_spaces {
                    let cheat_end_pos_time = match time_per_pos.get(&cheat_end_pos) {
                        ...
                        None => {
                            ...
                            next_grid.start_pos = cheat_end_pos;

                            match next_grid.compute_best_path() {
                                Some(next_path) => {
                    ...
                    if let Some(cheat_end_pos_time) = cheat_end_pos_time {
                        ...
                        let total_time = path_time + cheat_distance + cheat_end_pos_time;
    ...
    fn compute_best_path(&self) -> Option<PathContext> {
        // Initial path context
        let path_context = PathContext {
            current_pos: self.start_pos,
            ordered_path: vec![self.start_pos],
        ...
        loop {
            ...
            for move_dir in DIRECTIONS {
                ...
                if self.is_empty_cell(&next_pos) {
                    ...
                    if let Some(cell_score) = best_cell_score.get(&next_path_context.current_pos) {
                        if next_path_context.time > *cell_score {
                            continue;
                        }
                    }

                    best_cell_score.insert(next_path_context.current_pos, next_path_context.time);
...
fn main() {
    ...
    let cheats_count = grid.get_cheats_count(2, time_saved_min);
    println!("Cheats count (2 picoseconds) = {cheats_count}");

    let cheats_count = grid.get_cheats_count(20, time_saved_min);
